...
const ParentSelector = ({ nodes, onSelect, defaultValue }: { nodes: FamilyNode[]; defaultValue?: string; onSelect: (value: string) => void }) => {
	return (
		<Select defaultValue={defaultValue} disabled={nodes.length === 0} onValueChange={onSelect}>
...
export default function Editor({ editingNode, nodes, setNodes, openEditor, setOpenEditor }: Props) {
			...
			<DialogContent>
				...
				<div className='grid gap-4 py-4'>
					<div className='grid grid-cols-4 items-center gap-4'>
						...
						<Input onChange={(e) => setNodeValue('name', e.currentTarget.value)} id='name' defaultValue={editingNode?.name} className='col-span-3' />
					</div>
					<div className='grid grid-cols-4 items-center gap-4'>
						<Label htmlFor='sex' className='text-right'>
							Jenis Kelamin
						</Label>
						<RadioGroup id='sex' orientation='horizontal' name='sex' required className='flex flex-row' defaultValue={editingNode?.sex ?? 'P'} onValueChange={(e) => setNodeValue('sex', e)}>
					...
					<div className='grid grid-cols-4 items-center gap-4'>
						...
						<Input defaultValue={editingNode?.spouse} id='spouse' onChange={(e) => setNodeValue('spouse', e.currentTarget.value)} placeholder='Opsional' className='col-span-3' />
					</div>
					<div className='grid grid-cols-4 items-center gap-4'>
						<Label htmlFor='notes' className='text-right'>
							Catatan
						</Label>
						<Input id='notes' defaultValue={editingNode?.notes} onChange={(e) => setNodeValue('notes', e.currentTarget.value)} placeholder='Opsional' className='col-span-3' />
					</div>
				</div>
				<DialogFooter>
					<Button
						disabled={!editingNode && (node.name === '' || (node.parentId === undefined && nodes.length > 0))}
						onClick={() => {
							if (editingNode !== null) {
